...
<div class="iobject iobject--picture iobject--picture-card card">
	<figure class="">
		{if $render_link}<a class="iobject--picture__link image-wrap" href="{!$picture|img_url:$geometry_detail}" title="{$picture->getTitle()} - {t}Enlarge image{/t}" data-pswp-width="{$picture|img_width:$geometry_detail}" data-pswp-height="{$picture|img_height:$geometry_detail}">{else}<div class="image-wrap">{/if}
			<picture>
	...
	sizes="(max-width:1400px) 100vw, 1400px">
		...
		<figcaption class="card-body iobject__caption">
			...
			<div class="iobject__title card-title">{if $render_link}<a class="iobject--picture__link js_picture_trigger" href="{!$picture|img_url:$geometry_detail}" title="{if $picture->getTitle()}{$picture->getTitle()}{/if} - {t}Enlarge image{/t}">{!"search-plus"|icon}</a> <span class="iobject__title__separator">|</span> {/if}{$picture->getTitle()}</div>
			{/if}
